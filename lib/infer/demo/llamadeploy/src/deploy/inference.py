...
def model_fn(
    model_dir: str,
) -> tuple[PreTrainedModel | GenerationMixin, PreTrainedTokenizerBase]:
    loaded_model = AutoModelForCausalLM.from_pretrained(model_dir, device_map="auto")
...
def predict_fn(
    ...
    model_and_tokenizer: tuple[
        PreTrainedModel | GenerationMixin, PreTrainedTokenizerBase
    ],
) -> list[dict[str, Any]]:
    model, tokenizer = model_and_tokenizer
    prompt = data.pop("inputs", data)
    inputs = tokenizer(prompt, return_tensors="pt").to(
        cast(PreTrainedModel, model).device
    )
